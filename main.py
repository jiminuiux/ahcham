...
async def get_weather_advice(city: str, date: str = "today", schedule: str = "") -> str:
    ...
    Args:
        ...
        schedule: 캘린더 일정 전체 텍스트 (예: "오후 2시 팀 미팅 / 서류 챙기기 / 저녁 약속")
    """
    ...
    # 일정 파싱
    schedule_comment, calendar_items = parse_schedule(schedule)
    ...
    result += f"🌤️ 날씨: {weather_desc}\n"
    result += f"🌡️ 기온: 최고 {temp_max}°C / 최저 {temp_min}°C\n"
    result += f"🌧️ 강수 확률: {rain_prob}%\n"
    result += f"💨 바람: {wind_speed} km/h\n"

    # 일정 섹션 (항상 표시)
    if schedule:
        result += f"\n📅 {date_label} 일정: {schedule}\n"
        result += f"{schedule_comment}\n"
    else:
        result += f"\n📅 {date_label} 일정: 등록된 일정이 없어요.\n"
...
def parse_schedule(schedule: str):
    ...
    if any(kw in s for kw in FORMAL_KEYWORDS):
        comment = "- 중요한 일정이 있으니 단정한 스타일을 추천해요!"
    elif any(kw in s for kw in OUTDOOR_KEYWORDS):
        comment = "- 야외 활동이 있으니 활동하기 편한 옷차림을 추천해요!"
    else:
        comment = "- 옷차림에 특별한 제약이 없는 일정이에요. 날씨 기반으로만 추천드릴게요!"

    # 준비물 감지 (캘린더에 "챙겨야 할 것" 류 키워드 포함된 항목)
...
def recommend(temp_max, temp_min, rain_prob, wind_speed, weather_code, schedule):
    ...
    if schedule:
        s = schedule.lower()
        if any(kw in s for kw in FORMAL_KEYWORDS):
            outfit_parts.append("- 포멀한 스타일 추천")
