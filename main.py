...
app = FastAPI(title="날씨요정 MCP Server")

# ─── MCP 메타정보 ───────────────────────────────────────────────
@app.get("/")
async def root():
    return {"name": "날씨요정", "version": "1.0.0"}


@app.get("/.well-known/mcp")
async def mcp_info():
    """PlayMCP '정보 불러오기' 버튼이 이 엔드포인트를 읽어요."""
    return {
        "name": "날씨요정",
        "identifier": "weatherFairy",
        "description": "날씨요정이 매일 아침 오늘 날씨와 일정을 보고, 뭘 입고 뭘 챙길지 알려드려요",
        "version": "1.0.0",
        "tools": [
            {
                "name": "weatherFairy_getWeatherAdvice",
                "description": "도시명과 날짜(오늘/내일)를 받아 날씨 기반 옷차림과 준비물을 추천합니다.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "city": {
                        ...
                        "date": {
                            "type": "string",
                            "enum": ["today", "tomorrow"],
                            "description": "오늘(today) 또는 내일(tomorrow)"
                        },
                        "schedule": {
                            "type": "string",
                            "description": "오늘 일정 (선택사항, 예: 오후 외근, 중요한 발표)"
                        }
                    },
                    "required": ["city", "date"]
                }
...
async def call_tool(req: ToolCallRequest):
    if req.tool == "weatherFairy_getWeatherAdvice":
        return await get_weather_advice(req.parameters)
    return JSONResponse(status_code=404, content={"error": f"Unknown tool: {req.tool}"})


# ─── 날씨 조회 + 추천 로직 ───────────────────────────────────────
async def get_weather_advice(params: dict) -> dict:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        if not geo_data.get("results"):
            return {"content": f"'{city}' 도시를 찾을 수 없어요. 다시 입력해 주세요."}
    ...
    result = f"""🧚 날씨요정의 {date_label} 브리핑 ({city})
